...
class Upload_Controller extends CI_Controller {
    ...
    public function do_upload($user_name = '') {
        ...
        $this->form_validation->set_rules('3Dfile', '3D model', 'required');
        $this->form_validation->set_rules('XMLfile', 'Data file', 'required');

        
        if ($this->form_validation->run() == TRUE) {
            $this->load->view('upload/file_view');
        } else {
            ...
            $tmp_folder_name = "./uploads/" . $file_name;
            mkdir($tmp_folder_name);

            $model_config_model = array(
                'upload_path' => $tmp_folder_name,
                'allowed_types' => "obj|jpg|png",
                'overwrite' => TRUE,
                'max_size' => "10240000"
            );

            $model_config_data = array(
                'upload_path' => $tmp_folder_name,
                'allowed_types' => "xml|jpg|png",
                'overwrite' => TRUE,
                'max_size' => "10240000"
            );

            $this->load->library('upload', $model_config_model);
            ...
            $this->load->library('upload', $model_config_data);
